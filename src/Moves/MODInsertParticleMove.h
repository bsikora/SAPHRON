...
namespace SAPHRON
{
	...
	class MODInsertParticleMove : public Move
	{
	private: 
		Rand _rand;
		int _rejected;
	...
	public:
		MODInsertParticleMove(const std::vector<int>& species,
		...
		MODInsertParticleMove(const std::vector<std::string>& species,
							 ...
							 const MoveOverride& override) override
		{
			...
			// for extra K particle in the system

			size_t n_ext = _extraCount.size();
			assert(n_ext > 0);
			auto type_ext = _rand.int32() % n_ext;
			plist_ext[0] = w->UnstashParticle(_extraCount[type_ext]);
		...
		{
			return new MODInsertParticleMove(static_cast<const MODInsertParticleMove&>(*this));
		}

		~MODInsertParticleMove(){}
	};
}
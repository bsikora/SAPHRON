...
int main(int narg, char **arg)
{
  ...
  results_file.open("chemical_pot" + std::to_string(debye)+"_results.dat", std::ofstream::out);
  results_file<<"Na"<<std::endl;
  results_file.close();
...
*/
  ...
  sodium->SetCharge(0.0); // dereference the pointer and set the charge
...
/*
  ...
  */
  ...
  InsertParticleMove Ins1({{"Sodium"}}, WM, 20, true,seed + 3);
  //InsertParticleMove Ins2({{"Hydroxide"},{"Sodium"}}, WM, 20, true,seed + 30);
  DeleteParticleMove Del1({{"Sodium"}}, true, seed + 4);
...
void WriteResults(World &world, std::ofstream& results_file, double &debye)
{
  ...
  if (rank == 0)
  {
    ...
    results_file<<mu_ex_Na<<std::endl;
    results_file.close();
  }
}
